...
class Task(str, Enum):
    EXAMPLE_TASK = 'mlc_algo_efficiency_mnist_jax'


def mlc_algo_efficiency_mnist_jax(task_args: List[str]) -> None:
    ...
    with open(args.output_file, 'w') as stream:
        stream.write(f"Running mlc_algo_efficiency_mnist_jax task with parameters '{config}'.\n")
...
def main():
    ...
    if mlcube_args.mlcube_task == Task.EXAMPLE_TASK:
        mlc_algo_efficiency_mnist_jax(task_args)
    else:
